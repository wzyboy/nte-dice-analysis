...
Windows 便携版 ZIP 为了兼容性和下载体积，只包含 CPU 运行时。

第一次运行 OCR 可能需要几分钟，并且需要联网下载默认的 PP-OCRv5 模型。运行日志会写入用户文档目录下的
...
如果需要处理大量截图，GPU 用户可以自行从源码运行。不要在同一个环境中同时安装两个 OCR 运行时 extra。
...
源码运行仍可使用 `--extra gpu` 选择 GPU Paddle 运行时；release ZIP 只包含 CPU 运行时，因为它更小，也能在更多
Windows 机器上运行。有 NVIDIA 显卡的用户可以自行配置源码环境，大批量 OCR 时大约可获得 20 倍速度提升。
